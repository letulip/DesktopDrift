...
const CFG = {
  ...
  grip: 0.97,           // боковое сцепление: ближе к 1 = больше скольжения
...
// степень dt*PHYS_HZ → одинаковое ощущение на любой частоте.
// Эталон = 60 Гц: «скользкое», дрифтовое поведение (боковая скорость живёт
// дольше). Меньше значение = ещё больше «болтанки»; больше — цепче/«суше».
const PHYS_HZ = 60;
...
const CARS = [
  { name: 'Bismark', len: 82, body: '#474d56', stroke: '#222222', vw: 426, vh: 157, flip: true,
    drive: { thrust: 580, maxSpeed: 470, steer: 2.2, steerSmooth: 4.5, lowSpeedTurn: 0.20, selfAlign: 0.82, grip: 0.972, driftSteerBoost: 1.2 },
    details: [
  ...
  { name: 'Panda', len: 75, body: '#eef0f2', stroke: '#111111', vw: 402, vh: 157, flip: true,
    drive: { thrust: 620, maxSpeed: 410, steer: 3.0, steerSmooth: 8, lowSpeedTurn: 0.30, selfAlign: 0.82, grip: 0.975, driftSteerBoost: 1.3 },
    details: [
...
function frame(now) {
  ...
  const fAdj = dt * PHYS_HZ;   // показатель для покадровых множителей: =1 на 60 Гц, =0.5 на 120 Гц
