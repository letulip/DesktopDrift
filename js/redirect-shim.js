// Redirect shim for the legacy per-page URLs → the SPA shell hash route (SPA Phase B/C).
// Each old <page>.html now loads only this module; it maps the page + its query string to
// index.html#/<screen>?... and replaces the history entry. Keeps bookmarks / deep links / the
// game's on-error redirects working after the markup moved into index.html's <template>s. ALL legacy
// pages are shimmed now — game / sandbox / donate too — so any stray *.html link opens in the shell.
import { routeToHash } from './route.js';

const PAGE_TO_SCREEN = {
  'tracks.html': 'tracks', 'zen.html': 'zen', 'select.html': 'select',
  'modify.html': 'modify', 'settings.html': 'settings', 'achievements.html': 'achievements',
  'game.html': 'game', 'sandbox.html': 'game', 'donate.html': 'donate',
};

const page = location.pathname.split('/').pop() || '';
const screen = PAGE_TO_SCREEN[page] || 'menu';
const q = new URLSearchParams(location.search);
// sandbox.html carries no query but IS the game screen in sandbox mode — inject it.
const mode = q.get('mode') || (page === 'sandbox.html' ? 'sandbox' : null);

location.replace('index.html' + routeToHash(screen, {
  track:   q.get('track'),
  mode,
  dir:     q.get('dir') === 'rev' ? 'rev' : null,
  car:     q.get('car'),
  dpr:     q.get('dpr'),
  surface: q.get('surface'),
}));
