...
import {
  ITEM_KNIFE_1, ITEM_PLATE_YELLOW, ITEM_STAPLER, ITEM_CUP,
  ITEM_FORK_1,  ITEM_CUTTER,       ITEM_NOTEBOOK, ITEM_PENCIL_PLUS, ITEM_CLIPBOARD,
} from './items.js';
...
// ang: 0 = длинная ось горизонтально (вправо); π/2 ≈ 1.57 = вертикально (вверх).
// Углы выбраны по направлению трека в каждой точке (параллельно ближайшей стенке).
const PLACED_ITEMS = [
  // Левый внешний изгиб — трек идёт с ЮЗ на СЗ, нож лежит вдоль стенки
  { ...ITEM_KNIFE_1,      x: -1211, y:  -255, ang:  1.3  },
  // Широкий левый карман — тарелка круглая, вращение не важно
  { ...ITEM_PLATE_YELLOW, x:  -831, y:     5, ang:  0    },
  // Верхний левый угол — степлер наискосок (трек поворачивает вправо)
  { ...ITEM_STAPLER,      x:  -491, y:  -430, ang: -0.5  },
  // Левый центр — чашка круглая
  { ...ITEM_CUP,          x:  -285, y:   141, ang:  0    },
  // Нижняя прямая — трек почти горизонтален, вилка вдоль него
  { ...ITEM_FORK_1,       x:  -220, y:   621, ang:  0.15 },
  // Верхняя внутренняя дуга — трек горизонтален, нож почти вдоль
  { ...ITEM_CUTTER,       x:  -200, y:  -619, ang:  0.2  },
  // Самый верх карты — блокнот вдоль горизонтали
  { ...ITEM_NOTEBOOK,     x:    39, y:  -853, ang:  0.1  },
  // Правый верх — трек поворачивает вниз, pencil по диагонали
  { ...ITEM_PENCIL_PLUS,  x:   420, y:  -500, ang: -0.6  },
  // Правый внутренний изгиб — трек идёт с СВ на ЮВ, клипборд вдоль стенки
  { ...ITEM_CLIPBOARD,    x:   884, y:  -253, ang:  1.2  },
];
