...
test('carLook: lazily returns a null-default look per car index', () => {
  assert.deepEqual(carLook(0), { bodyColor: null, neonColor: null, finish: null, trailColor: null, glassColor: null, outlineColor: null, neon: null, expression: null });
});

test('defaults: settings / records / achievements', () => {
  assert.deepEqual(settings(), { units: 'kmh', haptics: true, soundEnabled: true, volume: 1.0 });
...
test('mutate live object + save() persists correct JSON shape', () => {
  ...
  assert.deepEqual(raw.settings, { units: 'kmh', haptics: true, soundEnabled: true, volume: 1.0 });
});
